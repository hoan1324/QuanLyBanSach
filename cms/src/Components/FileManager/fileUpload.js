...
function FileUpload({ folderId, fetchDataFile }) {
  ...
  const { imageTypes, documentTypes, videoTypes, audioTypes, compressedTypes } =
    constantType.extension;
  ...
  const customRequest = async (options) => {
    ...
    try {
      ...
      const response = await actionDiffirent(service, methodName, [
        folderId,
        formData,
      ]);
  ...
  const beforeUpload = (file) => {
    ...
    if (fileSizeMB > fileSizeLimits[fileType]) {
      message.error(
        `${file.name} vượt quá giới hạn ${fileSizeLimits[fileType]}MB!`
      );
      return false;
    }
